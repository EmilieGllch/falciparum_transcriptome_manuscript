...
HMMER3 = "/usr/local/bioinfsoftware/hmmer/hmmer-3.1b1/bin/hmmsearch"

def split_easy_from_hard(inputfile, outputdir, num_stops):
  seqCount = 0
  badSeqs = 0
  bad_lengths = []
...
def main():
  ...
  parser.add_option("", "--numStops", dest="num_stops", type=int
      , default=2
      , help="number of stops allowed before searching for ORFs (default=2)")
  ...
  easy, hard = split_easy_from_hard(options.contig_file
    , options.outputdir, options.num_stops)
