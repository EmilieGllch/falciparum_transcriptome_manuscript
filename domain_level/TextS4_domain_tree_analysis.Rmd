...
sigUp_norm_counts <- norm_counts[rownames(norm_counts) %in% rownames(resUp),]

percent_counts <- t(t(sigUp_norm_counts)/colSums(norm_dom_counts))

sigUp_norm_counts_df <- melt(percent_counts)
colnames(sigUp_norm_counts_df) <- c("Domain Cluster", "Isolate", "Proportion of expression")
sigUp_norm_counts_df$disease[grepl("^S.*",sigUp_norm_counts_df$Isolate)]<-"severe"
sigUp_norm_counts_df$disease[!grepl("^S.*",sigUp_norm_counts_df$Isolate)]<-"non-severe"
colnames(sigUp_norm_counts_df) <- c("Domain Cluster", "Isolate", "Proportion of VAR expression", "Phenotype")
...
gg <- gg + facet_wrap(~`Domain Cluster`, scales = "free_y") + theme_bw()
gg <- gg + scale_colour_manual(values = colors)
gg
```
